#Adapted from original RobotFlamingo by Richard Bornemann
...
class DeterministicDecoderSeq(ActionDecoder):
    '''Code adapted from https://github.com/RoboFlamingo/RoboFlamingo/blob/main/robot_flamingo/models/action_head.py
    This implementation resets the hidden state only at the beginning of a new episode, not for each window.'''
    ...
    def __init__(
        ...
        super(DeterministicDecoderSeq, self).__init__()
        self.fc_state = None
    ...
    def forward(  # type: ignore
        ...
        hidden_state_mask: Optional[torch.Tensor] = None,
        ...
        if not isinstance(self.rnn, nn.Sequential) and isinstance(self.rnn, nn.RNNBase):
            ...
            if input_feature.shape[1] == 1:
                ...
                #Set hidden state to 0 for all episodes in the batch who begin in this window
                if self.hidden_state is None:
                    self.hidden_state = h_0
                else:
                    if hidden_state_mask is not None:
                        h_t = torch.mul(self.hidden_state[0], hidden_state_mask.reshape(1, -1, 1).repeat(1, 1, self.hidden_size))
                        c_t = torch.mul(self.hidden_state[1], hidden_state_mask.reshape(1, -1, 1).repeat(1, 1, self.hidden_size))
                        self.hidden_state = (h_t.detach(), c_t.detach())
                x, h_n = self.rnn(input_feature, self.hidden_state)
                self.hidden_state = h_n
                if self.last_action:
                    x = x[:, -1].unsqueeze(1)
